...
namespace DoubleDoubleStatistic.Optimizer {
    internal class GridMaximizeSearch2D : IConvexSearch<(ddouble x, ddouble y)> {
        ...
        public GridMaximizeSearch2D(Func<(ddouble x, ddouble y), ddouble> cost_func, (ddouble min, ddouble max) range_x, (ddouble min, ddouble max) range_y, int depth, int max_iter) {
        ...
        public (ddouble x, ddouble y) BestParam => searched.Count >= 1
            ? searched.MaxBy(item => item.Value.cost).Value.param
        ...
        private ddouble CostCache((Int128 x, Int128 y) position, (ddouble x, ddouble y) param) {
            ...
            if (!searched.TryGetValue(position, out ((ddouble x, ddouble y) param, ddouble cost) value)) {
                ...
                cost = ddouble.IsNaN(cost) ? ddouble.NegativeInfinity : cost;
        ...
        public static (ddouble x, ddouble y) Search(Func<(ddouble x, ddouble y), ddouble> cost_func, ((ddouble x, ddouble y) min, (ddouble x, ddouble y) max) range, int iter) {
            GridMaximizeSearch2D searcher = new(cost_func, (range.min.x, range.max.x), (range.min.y, range.max.y), depth: 100, max_iter: iter);
