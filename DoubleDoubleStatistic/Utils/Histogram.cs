...
namespace DoubleDoubleStatistic.Utils {
    public class Histogram {
        ...
        public Histogram(ReadOnlyCollection<double> samples, int bins, (double min, double max) range) {
            ...
            double xr = range.max - range.min, h = xr / bins;
            ...
            foreach (double sample in samples) {
                ...
                int index = int.Min(bins - 1, (int)double.Clamp(double.Floor((sample - range.min) / h), 0d, bins));
            ...
            Bins = bins;
            BinsEdge = new ReadOnlyCollection<ddouble>((new ddouble[bins + 1]).Select((_, idx) => range.min + (ddouble)xr * idx / bins).ToArray());
            BinsCentor = new ReadOnlyCollection<ddouble>((new ddouble[bins]).Select((_, idx) => range.min + (ddouble)xr * (idx + 0.5d) / bins).ToArray());
        ...
        public Histogram(IEnumerable<double> samples, IEnumerable<double> bins_edge)
            : this(
                  new ReadOnlyCollection<double>(samples.ToArray()),
                  new ReadOnlyCollection<double>(bins_edge.ToArray())
            ) { }

        public Histogram(IEnumerable<double> samples, ReadOnlyCollection<double> bins_edge)
            : this(new ReadOnlyCollection<double>(samples.ToArray()), bins_edge) { }

        public Histogram(ReadOnlyCollection<double> samples, IEnumerable<double> bins_edge)
            : this(samples, new ReadOnlyCollection<double>(bins_edge.ToArray())) { }
        ...
        public Histogram(ReadOnlyCollection<double> samples, ReadOnlyCollection<double> bins_edge) {
            ...
            if (!bins_edge.All(double.IsFinite)) {
                throw new ArgumentException("Invalid bins: finite", nameof(bins_edge));
            }

            double xmin = bins_edge[0], xmax = bins_edge[^1];
            ...
            ddouble[] density = counts.Select((n, idx) => n / (ddouble)((bins_edge[idx + 1] - bins_edge[idx]) * samples.Count)).ToArray();

            Bins = bins_edge.Count - 1;
            BinsEdge = new ReadOnlyCollection<ddouble>(bins_edge.Select(v => (ddouble)v).ToArray());
            BinsCentor = new ReadOnlyCollection<ddouble>(counts.Select((_, idx) => (ddouble)(bins_edge[idx + 1] + bins_edge[idx]) * 0.5d).ToArray());
            Counts = new ReadOnlyCollection<int>(counts);
            Density = new ReadOnlyCollection<ddouble>(density);
        }
        ...
        public Histogram(IEnumerable<ddouble> samples, IEnumerable<ddouble> bins_edge)
            : this(
                  new ReadOnlyCollection<ddouble>(samples.ToArray()),
                  new ReadOnlyCollection<ddouble>(bins_edge.ToArray())
            ) { }
